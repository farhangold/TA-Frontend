...
const Sidebar = () => {
  ...
  // Check if the current path starts with an item path
  // This helps with active state highlighting for nested routes
  const isActive = (path: string) => {
    return pathname === path || pathname.startsWith(`${path}/`);
  };

  return (
    <div className="fixed h-screen bg-white/80 backdrop-blur-lg border-r border-gray-200/50 shadow-xl w-[200px] md:w-[200px] sm:w-[180px] flex flex-col overflow-hidden z-50">
          ...
          <ul className="space-y-1 px-2">
            {menuItems.map((item, index) => (
              <li key={index}>
                <Link
                  href={item.path}
                  className={`flex items-center px-4 py-3 text-sm rounded-r-xl transition-all duration-200 ${
                    isActive(item.path)
                ...
                >
                  ...
                  <span className={isActive(item.path) ? "font-semibold" : "font-medium"}>
                    {item.label}
                  </span>
                </Link>
              </li>
            ...
            {adminMenuItems.length > 0 && (
              <>
                <li className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase mt-4 mb-2">
                  Admin
                </li>
